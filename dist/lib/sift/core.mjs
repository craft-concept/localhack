...
export const root = () => {
    ...
    send.next = () => { };
    send.meta = (...fns) => {
        for (const fn of iter(fns))
            send.next = fn(send) || send.next;
        return send;
    };
    return send;
...
export const originalPlugin = ({ send }) => inputs => {
    ...
    send.sending = true;
    (_a = send.state) !== null && _a !== void 0 ? _a : (send.state = {});
    const result = produce(inputs, inputs => {
        send.state = produce(send.state, state => {
            var _a;
            (_a = state.plugins) !== null && _a !== void 0 ? _a : (state.plugins = []);
     ...
     */
    const queued = (_b = send.queue) === null || _b === void 0 ? void 0 : _b.shift();
    if (queued)
        send(queued);
    return result;
};
...
test(make, ({ eq }) => {
    const send = sift(input => state => {
        var _a;
        (_a = state.count) !== null && _a !== void 0 ? _a : (state.count = 0);
        state.count++;
    }, input => { }, input => (input.testing = true), input => state => send => {
        if (state.count === 4)
            send({ msg: "count is 4!" });
    });
